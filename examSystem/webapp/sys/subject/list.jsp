	...
	<div class="container-fluid">
		<div class="row-fluid">
			<form class="form-inline" method="post"
				action="<%=basePath%>SubjectServlet?cmd=limit">
				<input class="input-xlarge" placeholder="请输入需要查找的试题" name="scontent"
					type="text" value="${param.scontent}"> <input
					class="btn icon-search" type="submit" value="查询" /> <a
					class="btn btn-primary" href="<%=basePath%>sys/subject/add.jsp">
					<i class="icon-plus"></i> 新增试题
				</a>
			</form>
					...
					<tbody>
						<c:forEach items="${pager.list}" var="item">
							<tr>
								...
								<td><a
									href="<%=basePath%>SubjectServlet?cmd=update&sid=${item.sid}">修改</a>
									<a
									href="<%=basePath%>SubjectServlet?cmd=del&sid=${item.sid}">删除</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
					...
					<ul>
						...
						<c:forEach begin="${pager.minPageNo }" step="1"
							...
							<li><c:if test="${pager.currentPageNo==index}" var="t">
									...
									<!--注意要处理的servlet映射-->
									<a
										href="<%=basePath%>SubjectServlet?cmd=limit&currNo=${index}">${index}</a>
								</c:if></li>
						</c:forEach>
						...
						<li><c:if
								...
								<!--注意要处理的servlet映射-->
								<a
									href="<%=basePath%>SubjectServlet?cmd=limit&currNo=${pager.currentPageNo+1}">下一页</a>
							</c:if></li>
					</ul>
				</div>
			</div>

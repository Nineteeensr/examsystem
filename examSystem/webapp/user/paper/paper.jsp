...
<body>
	...
	<script>
		...
		function getScore(){
			//var spid = n
			$.post({
		        url: basePath + 'StudentPaperServlet?cmd=score&userid='+'${userid}'+'&spid='+now.getTime()+'&pname=${pname}',
		        contentType: false,
		        processData: false,
		        success: function(res) {
		            ...
		            layer.open({
						...
						icon:1,
							...
							location.href = basePath+'StudentPaperServlet?cmd=list';
						}
					})
		        },
		        error: function(res) {
		...
		function postAnswer(){
			...
			$('.subject').each(function(index){
				...
				console.log(datas)
				$.post({
					url: basePath+'StudentPaperServlet?cmd=answer&'+ datas,
					contentType: false,
					processData: false,
					success:function(){
						// 在最后一次提交完成后跳转到错题列表
